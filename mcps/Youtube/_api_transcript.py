...
On cloud environments (GCP, AWS, Azure), YouTube may block Innertube API
requests. In that case, get_transcript returns None and the caller should
fall back to alternative summarization strategies (e.g., Gemini YouTube
URL understanding).

Set SKIP_TRANSCRIPT_API=1 to skip youtube-transcript-api entirely (useful
on cloud deployments where it always fails, saving ~3s per video).
"""
...
_SKIP_API = os.getenv("SKIP_TRANSCRIPT_API", "").strip() in ("1", "true", "yes")
...
def get_transcript(video_id: str, language: str = None) -> str | None:
    ...
    Note: Returns None on cloud IPs where YouTube blocks Innertube API.
    """
    if _SKIP_API:
        return None
    ...
    try:
        ...
        print_error(f"[transcript] list failed for {video_id}: {e}")
        return None
    ...
    try:
        ...
        print_error(f"[transcript] fetch failed for {video_id}: {e}")
        return None
