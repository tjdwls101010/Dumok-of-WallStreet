...
3. **추가 프롬프트 결합**: 추가 프롬프트가 있으면 `<Additional_Prompt>` 태그로 추가
4. **AI 처리**: gemini-cli headless mode로 재구조화 실행
5. **출력 저장**: 결과를 `Restructured_` 접두사 파일로 저장
...
[INFO] 추가 프롬프트: 'Amos Decker'를 '에이머스 데커'라고 표현해.
...
`batch.sh`로 폴더 내 파일들을 10초 간격으로 백그라운드 실행:
...
**동작 방식**:
1. 폴더에서 `*.md` 파일 검색 (`Restructured_`, `.backup` 제외)
2. 파일명 기준 오름차순 정렬 (`sort -V`)
3. 각 파일에 `restructure.sh` 백그라운드 실행 (10초 간격으로 시작)
4. 완료 대기 없이 다음 파일 시작 → API rate limit 방지 + 처리 시간 단축
5. 마지막에 `wait`로 모든 작업 완료 대기
