...
- **Geographic control — moat or hostage?** The same concentration cuts both ways. A name's scarce node sitting in one controlled region is a *moat* when the name controls that node and prices it — but a *hostage* when the controlling **state** holds an export/permit lever over the name's *own* output. A US-listed materials maker that *manufactures inside* the controlled region is often **both at once** — beneficiary of the scarcity *and* trapped behind its government's permit wall — so net the two rather than reading the concentration one-directionally. (Macro's "export controls create monopolies" is the *outside-the-geography* case; invert it when the name sits *inside* it. Generalizes to any geo-concentrated critical input — rare earths, gallium, neon, lithium processing.)
...
But "what if the customer designs us out?" isn't answered once — it's *monitored*. Watch the leaks and OEM/CES disclosures that reveal a second source qualifying (the disclosure of a qualified supplier is real evidence, even though conference *hype* is noise); while the odds stay ambiguous, **hedge and cut concentration** rather than binary-exiting on the first rumor *or* doubling down on hope; re-rate the floor only when the switch is confirmed. The peak danger has a specific shape: **a prototype/dev-stage qualification is not a production-at-scale win.** Demo-locked designs routinely die at the mass-production cost-down when the customer re-optimizes the BOM — so a supplier qualified only into prototypes is *lower-conviction, higher-designed-out-risk* than one shipping at production scale (the §4 maturity lens, applied to the supplier *inside its customer's program*, not just to the name's own revenue). The mirror-image trap: a supplier *deduced* into a program by spec-matching may never have been confirmed at all — run §1's DEDUCED-vs-CONFIRMED check before committing conviction.

### Two pipeline hand-off flags — the verdict telling you where it punted
The composite verdict can carry two booleans that are *explicit hand-offs* — the pipeline marking where its own judgment stops. Read each as a question routed back to the gates above, never as a ruling:
- **`speculative_cap_applied`** — it held the grade no higher than HOLD because it read a cash-burner with no machine-readable moat. That is Step 2 handed straight back to you: override up *only* on a bottleneck you can actually show — a named design-win, a sole-source part, a gate the classifier couldn't see — and first rule out that the cap is a one-time charge (impairment, litigation, restructuring) dragging *trailing* margin below the line on a genuine business. Absent shown evidence, HOLD is the honest answer; the cap is a floor, not a cue to override in either direction — neither wave a capped cash-burner up on its story, nor slam it to AVOID when a real revenue ramp is keeping it alive (a ramp holds a capped name *off* AVOID without earning an override *up*).
- **`unsubstantiated_bottleneck`** — a bottleneck label sitting on a name that neither grows nor earns. It does *not* cap; it raises the §4 fork it cannot call from a snapshot: melting legacy (commoditizing, no recovery) or cyclical trough (capacity intact, margins about to inflect)? The point-in-time numbers are identical on both sides — only the revenue/margin *trajectory* and the designed-out monitor separate them. Resolve it on the path, not the snapshot. One behavioral resolver when even the trajectory is ambiguous: the *tenor of cash commitment*. A supplier that can demand **multi-year prepayment** to guarantee supply is facing a structural shortage, not a cyclical one — customers don't lock years of cash forward unless they fear being shut out (revealed preference, louder than any order-book number).
