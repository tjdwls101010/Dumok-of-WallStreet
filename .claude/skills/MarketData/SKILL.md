...
| `supply_chain` | SEC 10-K/10-Q supply chain intelligence extraction: suppliers, customers, single-source dependencies, geographic concentration, capacity constraints, supply chain risks, revenue concentration, geographic revenue, and purchase obligations with confidence scoring. Supports regex and LLM mode (Gemini structured output, default). LLM mode additionally extracts Item 8 Notes for quantitative data | `data_advanced/sec/supply_chain.py` |
