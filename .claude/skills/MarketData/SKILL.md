...
|----------|-------------|--------|
| `minervini` | Full SEPA pipeline (Pipeline-Complete, 5 subcommands: analyze, screen, market-leaders, compare, recheck): SEPA 5-element composite scoring (0-100) with hard-gate safety (Stage 2 + Trend Template 8/8), VCP/entry pattern synthesis, Code 33 and earnings quality integration, risk assessment with stop-loss and R:R calculation, position sizing suggestion, market leadership dashboard with distribution day counting, sector screening with fundamental overlay, multi-ticker SEPA comparison, and position management with tennis ball/egg classification | `pipelines/minervini/` |
...
| `serenity` | Full Serenity 6-Level pipeline (Pipeline-Complete, 3 subcommands: macro, analyze, discover): macro regime assessment, fundamental validation (5 health gates with 3-level severity + fundamental readiness codes), conditional SEC dilution verification, active_dilution FCF optical illusion test. Full-filing markdown LLM extraction via edgartools. Entire filing sent to Gemini (no section slicing). XBRL structured data supplements 4 quantitative categories (revenue concentration, geographic revenue, inventory composition, purchase obligations). De facto single-source detection, named customer extraction, relationship specificity, market risk disclosures. 20-F support for foreign private issuers (TSMC, ASML) with auto-fallback from 10-K. 6-Criteria Bottleneck scoring with geo-risk tiers, Notes-based quantitative override, Item 7A market risk reinforcement, inventory capacity boost, duration extraction, and expanded pattern matching. L2 CapEx Flow and L3 Bottleneck require agent-driven context. L6 Taxonomy requires LLM classification. Discover supports --industry and --sector options. Sector-agnostic | `pipelines/serenity.py` |
...
| `williams` | Full Williams short-term volatility breakout pipeline (Pipeline-Complete): trade-setup composite scoring (10 components, 100 points, hard/soft gates, bonus points), single-ticker deep analysis, multi-ticker pattern scanning with accuracy metadata, market context (bond trend+gold-bond+COT+TDW/TDM calendar), batch watchlist (provisional mode), quick dashboard, and position management recheck (exit signals + verdict) | `pipelines/williams.py` |
...
| `bottleneck_scorer` | Bottleneck financial validation: validate (4 health gates with 3-level severity + asymmetry score), batch (multi-ticker sorted), rank (Supply Dominance formula). Now part of serenity pipeline; called directly by discover | `pipelines/serenity/_scorer.py` |
...
Filings, insider trades, 13F, FTD, supply chain intelligence
...
| `supply_chain` | SEC 10-K/10-Q/20-F supply chain intelligence extraction via edgartools with XBRL supplementation: suppliers, customers, single-source dependencies, geographic concentration, capacity constraints, supply chain risks, revenue concentration, geographic revenue, purchase obligations, market risk disclosures (Item 7A), and inventory composition with confidence scoring. Full-filing markdown sent to Gemini (no section pre-extraction). XBRL structured data supplements 4 quantitative categories (revenue concentration, geographic revenue, inventory composition, purchase obligations). 20-F support for foreign private issuers with auto-fallback from 10-K | `data_advanced/sec/supply_chain.py` |
