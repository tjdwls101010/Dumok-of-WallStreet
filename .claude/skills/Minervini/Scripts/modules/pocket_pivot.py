...
def cmd_detect(args):
	...
	_compressed = {
		"demand_day_count": full_result.get("demand_day_count", full_result.get("pocket_pivot_count", 0)),
	}
	if "most_recent" in full_result:
		_compressed["most_recent"] = full_result["most_recent"]
	elif "most_recent_pp" in full_result:
		_compressed["most_recent"] = full_result["most_recent_pp"]
	if "base_context" in full_result:
		_compressed["base_context"] = full_result["base_context"]
	if "thresholds" in full_result:
		_compressed["thresholds"] = full_result["thresholds"]
