...
def cmd_check(args):
	...
	# Active-only view: keep only active signals for pipeline consumption
	active_only_result = {k: v for k, v in full_result.items()
						  if k not in ("symbol",)}
	active_only_signals = {k: v for k, v in signals.items()
						   if isinstance(v, dict) and v.get("active", False)}
	active_only_result["signals"] = active_only_signals
	full_result["active_only"] = active_only_result
