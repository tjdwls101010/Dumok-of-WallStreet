...
def cmd_track(args):
	...
	# Compressed view: filter null entries from trajectory, then cap to 3
	_filtered_trajectory = [
		entry for entry in trajectory
		if entry is not None and isinstance(entry, dict)
		and entry.get("gross") is not None
	]
	if len(_filtered_trajectory) > 3:
		_filtered_trajectory = _filtered_trajectory[:3]
	compressed = {k: v for k, v in result.items() if k not in ("symbol",)}
	compressed["trajectory"] = _filtered_trajectory
